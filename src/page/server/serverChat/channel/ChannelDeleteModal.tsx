...
export default function ChannelDeleteModal() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        channelState.deleteModalOpen &&
        !(e.target as HTMLElement).closest(".channel-delete-modal")
      ) {
  ...
  return (
    ...
    >
      ...
      <div
        className={"channel-delete-modal z-10 flex items-center justify-center"}
      >
        <div
          style={{ width: "450px" }}
          className={
        ...
        >
          ...
          <div className={"mb-4 px-4 text-start"}>
            정말
            <span className={"font-semibold"}> {channelState.name} </span>
            채널을 삭제할까요? 삭제하면 되돌릴 수 없어요.
          </div>
