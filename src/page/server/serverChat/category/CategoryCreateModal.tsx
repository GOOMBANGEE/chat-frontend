...
import useCategoryCreate from "../../../../hook/server/serverChat/category/useCategoryCreate.tsx";
...
export default function CategoryCreateModal() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        categoryState.createModalOpen &&
        !(e.target as HTMLElement).closest(".category-create-modal")
      ) {
        resetCategoryState();
      }
    };
  ...
  const handleCheckboxChange = () => {
    setCategoryState({
      createModalOptionOpen: !categoryState.createModalOptionOpen,
    });
  };

  const isButtonDisabled = !categoryState.createModalName;
  const buttonText = categoryState.createModalOptionOpen ? "만들기" : "다음";
  ...
  return (
    ...
    >
      ...
      <div className={"category-create-modal flex items-center justify-center"}>
        <div
          className={
        ...
        >
          ...
          <div className={"flex flex-col px-4 py-2 text-start text-gray-400"}>
            <div className={"mb-2 mt-4 text-xs font-semibold"}>
              카네고리 이름
            </div>
            <input
              onChange={(e) => {
                setCategoryState({ createModalName: e.target.value });
              }}
            ...
            <div className={"text-sm"}>
              카테고리를 비공개로 만들면 선택한 멤버들과 역할만 이 카테고리를 볼
              수 있어요. 이 설정은 이 카테고리에 동기화된 채널들에도 자동으로
              적용돼요.
            </div>
